...
import { MDBox, MDTypography } from '@cashier/components';
  ...
  return (
    <ConfiguratorRoot variant="permanent" ownerState={{ openConfigurator: true }}>
      ...
      <MDBox pt={0.5} pb={3} px={3}>
        ...
        {/*<Divider/>*/}
        {
          ['Style', 'Font', 'Button text', 'Layout', 'Payment methods'].map((item) => {
            return (
        ...
        {
          ['Billing Information', 'Policies', 'Coupons', 'Tax', 'Shipping', 'Delivery'].map((item) => {
            return (
              <React.Fragment key={item}>
                <MDBox key={item} display="flex" justifyContent="space-between" alignItems="center" lineHeight={1}>
                  <MDTypography variant="h6">{item}</MDTypography>

                  <Switch checked={darkMode} onChange={() => null}/>
                </MDBox>
                <Divider/>
              </React.Fragment>
            );
